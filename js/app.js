...
app.controller('MyController', function ($scope) {
    ...
    $scope.edit = function (item, index, list) {
        $scope.isEdit = true;
    ...
    $scope.DropInTodo = function (index, item, external, type) {
        item.isDone = false;
        return item;
    }
    ...
    $scope.updateTask = function () {
        debugger;
        if ($scope.task.title.length > 0) {
            $scope.editList[$scope.editIndex] = $scope.task;
            $scope.task = {};
            $scope.isEdit = false;
            $scope.editList = [];
        } else {
